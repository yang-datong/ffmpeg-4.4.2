...
#include "libavcodec/mathops.h" // IWYU pragma: export
  ...
  for (i = 0; i < nb_filtergraphs; i++) {
    ...
    for (j = 0; j < fg->nb_inputs; j++) {
      ...
      while (av_fifo_size(ifilter->frame_queue)) {
        AVFrame *frame;
        av_fifo_generic_read(ifilter->frame_queue, &frame, sizeof(*frame),
                             NULL);
        av_frame_free(&frame);
      }
...
static double adjust_frame_pts_to_encoder_tb(OutputFile *of, OutputStream *ost,
                                             AVFrame *frame) {
  double float_pts =
      AV_NOPTS_VALUE; // this is identical to frame.pts but with higher precision
...
static void do_video_out(OutputFile *of, OutputStream *ost,
  ...
  if (!next_picture) {
    ...
    nb0_frames =
        0; // tracks the number of times the PREVIOUS frame should be duplicated, mostly for variable framerate (VFR)
    nb_frames = 1;
...
static int transcode_subtitles(InputStream *ist, AVPacket *pkt, int *got_output,
  ...
  for (i = 0; i < nb_output_streams; i++) {
    ...
    if (!ost->pkt && !(ost->pkt = av_packet_alloc())) exit_program(1);
...
static int process_input_packet(InputStream *ist, const AVPacket *pkt,
  ...
  if (!ist->pkt && !(ist->pkt = av_packet_alloc())) return AVERROR(ENOMEM);
  avpkt = ist->pkt;
  ...
  if (!ist->saw_first_ts) {
    ...
    if (pkt && pkt->pts != AV_NOPTS_VALUE && !ist->decoding_needed) {
      ...
      ist->pts =
          ist->dts; //unused but better to set it to a value thats not totally wrong
    }
    ist->saw_first_ts = 1;
  }
