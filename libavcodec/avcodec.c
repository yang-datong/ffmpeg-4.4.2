...
#if FF_API_LOCKMGR
int av_lockmgr_register(int (*cb)(void **mutex, enum AVLockOp op)) { return 0; }
#endif
...
int attribute_align_arg avcodec_open2(AVCodecContext *avctx,
  ...
  if (avcodec_is_open(avctx)) return 0;
  ...
  if (!codec) codec = avctx->codec;

  if (avctx->extradata_size < 0 ||
      avctx->extradata_size >= FF_MAX_EXTRADATA_SIZE)
    return AVERROR(EINVAL);
  ...
  if (!avci->buffer_frame || !avci->buffer_pkt || !avci->es.in_frame ||
      !avci->ds.in_pkt || !avci->last_pkt_props || !avci->pkt_props) {
    ret = AVERROR(ENOMEM);
    goto free_and_end;
  }
  ...
  if (codec->priv_data_size > 0) {
    ...
    if (codec->priv_class &&
        (ret = av_opt_set_dict(avctx->priv_data, &tmp)) < 0)
      goto free_and_end;
  } else {
    avctx->priv_data = NULL;
  }
  if ((ret = av_opt_set_dict(avctx, &tmp)) < 0) goto free_and_end;
  ...
  if ((avctx->coded_width || avctx->coded_height || avctx->width ||
      ...
      (av_image_check_size2(avctx->coded_width, avctx->coded_height,
       ...
       av_image_check_size2(avctx->width, avctx->height, avctx->max_pixels,
  ...
  if ((avctx->codec->capabilities & AV_CODEC_CAP_EXPERIMENTAL) &&
    ...
    codec2 = av_codec_is_encoder(codec) ? avcodec_find_encoder(codec->id)
  ...
  if (av_codec_is_decoder(avctx->codec)) {
    if (!avctx->bit_rate) avctx->bit_rate = get_bit_rate(avctx);
    ...
    if (avctx->channel_layout) {
      ...
      if (!avctx->channels)
        ...
        av_log(avctx, AV_LOG_WARNING,
               "Channel layout '%s' with %d channels does not match specified "
    ...
    if (avctx->framerate.num > 0 && avctx->framerate.den > 0)
      avctx->time_base = av_inv_q(
          av_mul_q(avctx->framerate, (AVRational){avctx->ticks_per_frame, 1}));
#endif
  }
...
free_and_end:
  ...
  if (HAVE_THREADS && avci->thread_ctx) ff_thread_free(avctx);

  if (codec->priv_class && avctx->priv_data) av_opt_free(avctx->priv_data);
  av_opt_free(avctx);
...
void avcodec_flush_buffers(AVCodecContext *avctx) {
  ...
  if (av_codec_is_decoder(avctx->codec)) av_bsf_flush(avci->bsf);

#if FF_API_OLD_ENCDEC
  FF_DISABLE_DEPRECATION_WARNINGS
  if (!avctx->refcounted_frames) av_frame_unref(avci->to_free);
  FF_ENABLE_DEPRECATION_WARNINGS
#endif
}
...
av_cold int avcodec_close(AVCodecContext *avctx) {
  ...
  if (avcodec_is_open(avctx)) {
    if (CONFIG_FRAME_THREAD_ENCODER && avctx->internal->frame_thread_encoder &&
        avctx->thread_count > 1) {
      ff_frame_thread_encoder_free(avctx);
    }
    if (HAVE_THREADS && avctx->internal->thread_ctx) ff_thread_free(avctx);
    if (avctx->codec && avctx->codec->close) avctx->codec->close(avctx);
    ...
    while (av_fifo_size(avctx->internal->pkt_props) >=
      ...
      av_fifo_generic_read(avctx->internal->pkt_props,
                           ...
                           sizeof(*avctx->internal->last_pkt_props), NULL);
      av_packet_unref(avctx->internal->last_pkt_props);
    }
    ...
    if (avctx->hwaccel && avctx->hwaccel->uninit) avctx->hwaccel->uninit(avctx);
...
void avcodec_string(char *buf, int buf_size, AVCodecContext *enc, int encode) {
  ...
  if (enc->codec_type == AVMEDIA_TYPE_VIDEO &&
    ...
    snprintf(buf + strlen(buf), buf_size - strlen(buf),
             ", %d reference frame%s", enc->refs, enc->refs > 1 ? "s" : "");
  ...
  case AVMEDIA_TYPE_VIDEO: {
    ...
    if (enc->colorspace != AVCOL_SPC_UNSPECIFIED ||
      ...
      if (strcmp(col, pri) || strcmp(col, trc)) {
        new_line = 1;
        av_strlcatf(detail, sizeof(detail), "%s/%s/%s, ", col, pri, trc);
      } else
    ...
    if (enc->width) {
      ...
      snprintf(buf + strlen(buf), buf_size - strlen(buf), "%dx%d", enc->width,
               enc->height);

      if (av_log_get_level() >= AV_LOG_VERBOSE &&
          (enc->width != enc->coded_width || enc->height != enc->coded_height))
        snprintf(buf + strlen(buf), buf_size - strlen(buf), " (%dx%d)",
      ...
      if (enc->sample_aspect_ratio.num) {
        av_reduce(&display_aspect_ratio.num, &display_aspect_ratio.den,
                  ...
                  1024 * 1024);
        snprintf(buf + strlen(buf), buf_size - strlen(buf),
                 " [SAR %d:%d DAR %d:%d]", enc->sample_aspect_ratio.num,
                 enc->sample_aspect_ratio.den, display_aspect_ratio.num,
    ...
    if (encode) {
      snprintf(buf + strlen(buf), buf_size - strlen(buf), ", q=%d-%d",
               enc->qmin, enc->qmax);
    } else {
  ...
  case AVMEDIA_TYPE_AUDIO:
    ...
    if (enc->sample_rate) {
      snprintf(buf + strlen(buf), buf_size - strlen(buf), "%d Hz, ",
               enc->sample_rate);
    }
    ...
    if (enc->sample_fmt != AV_SAMPLE_FMT_NONE &&
      ...
      snprintf(buf + strlen(buf), buf_size - strlen(buf), ", %s", str);
    }
    if (enc->bits_per_raw_sample > 0 &&
        enc->bits_per_raw_sample !=
            av_get_bytes_per_sample(enc->sample_fmt) * 8)
      snprintf(buf + strlen(buf), buf_size - strlen(buf), " (%d bit)",
               enc->bits_per_raw_sample);
    if (av_log_get_level() >= AV_LOG_VERBOSE) {
      if (enc->initial_padding)
        snprintf(buf + strlen(buf), buf_size - strlen(buf), ", delay %d",
                 enc->initial_padding);
      if (enc->trailing_padding)
        snprintf(buf + strlen(buf), buf_size - strlen(buf), ", padding %d",
  ...
  if (bitrate != 0) {
    snprintf(buf + strlen(buf), buf_size - strlen(buf), ", %" PRId64 " kb/s",
             bitrate / 1000);
