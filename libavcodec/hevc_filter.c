...
static int get_qPy_pred(HEVCContext *s, int xBase, int yBase, int log2_cb_size)
{
    ...
    int ctb_size_mask        = (1 << s->ps.sps->CtbLog2SizeY) - 1;
    int MinCuQpDeltaSizeMask = (1 << (s->ps.sps->CtbLog2SizeY -
                                      s->ps.pps->diff_cu_qp_delta_depth)) - 1;
    int xQgBase              = xBase - (xBase & MinCuQpDeltaSizeMask);
    int yQgBase              = yBase - (yBase & MinCuQpDeltaSizeMask);
    int min_cb_width         = s->ps.sps->min_cb_width;
    int x_cb                 = xQgBase >> s->ps.sps->log2_min_luma_coding_block_size;
    int y_cb                 = yQgBase >> s->ps.sps->log2_min_luma_coding_block_size;
...
static void sao_filter_CTB(HEVCContext *s, int x, int y)
{
    ...
    for (c_idx = 0; c_idx < (s->ps.sps->chroma_format_idc ? 3 : 1); c_idx++) {
        ...
        int ctb_size_h = (1 << (s->ps.sps->CtbLog2SizeY)) >> s->ps.sps->hshift[c_idx];
        int ctb_size_v = (1 << (s->ps.sps->CtbLog2SizeY)) >> s->ps.sps->vshift[c_idx];
...
static void deblocking_filter_CTB(HEVCContext *s, int x0, int y0)
{
    ...
    int log2_ctb_size = s->ps.sps->CtbLog2SizeY;
