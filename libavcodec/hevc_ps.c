...
static void remove_sps(HEVCParamSets *s, int id) {
  int i;
  if (s->sps_list[id]) {
    if (s->sps == (const HEVCSPS *)s->sps_list[id]->data) s->sps = NULL;
...
static void remove_vps(HEVCParamSets *s, int id) {
  int i;
  if (s->vps_list[id]) {
    if (s->vps == (const HEVCVPS *)s->vps_list[id]->data) s->vps = NULL;
...
int ff_hevc_decode_short_term_rps(GetBitContext *gb, AVCodecContext *avctx,
                                  ShortTermRPS *rps, const HEVCSPS *sps,
                                  int is_slice_header) {
  ...
  if (rps != sps->st_rps && sps->nb_st_rps) rps_predict = get_bits1(gb);
  ...
  if (rps_predict) {
    ...
    for (i = 0; i <= rps_ridx->num_delta_pocs; i++) {
      ...
      if (!used) use_delta_flag = get_bits1(gb);
...
static int decode_profile_tier_level(GetBitContext *gb, AVCodecContext *avctx,
  ...
  if (get_bits_left(gb) < 2 + 1 + 5 + 32 + 4 + 43 + 1) return -1;
...
static int parse_ptl(GetBitContext *gb, AVCodecContext *avctx, PTL *ptl,
...
static int decode_hrd(GetBitContext *gb, int common_inf_present,
  ...
  if (common_inf_present) {
    ...
    if (nal_params_present || vcl_params_present) {
      ...
      if (subpic_params_present) skip_bits(gb, 4); // cpb_size_du_scale
  ...
  for (i = 0; i < max_sublayers; i++) {
    ...
    if (!fixed_rate) fixed_rate = get_bits1(gb);
...
int ff_hevc_decode_nal_vps(GetBitContext *gb, AVCodecContext *avctx,
  ...
  if (!vps_buf) return AVERROR(ENOMEM);
  ...
  if (parse_ptl(gb, avctx, &vps->ptl, vps->vps_max_sub_layers) < 0) goto err;
  ...
  for (; i < vps->vps_max_sub_layers; i++) {
    ...
    if (vps->vps_num_reorder_pics[i] > vps->vps_max_dec_pic_buffering[i] - 1) {
      ...
      if (avctx->err_recognition & AV_EF_EXPLODE) goto err;
    }
  }
  ...
  if (vps->vps_timing_info_present_flag) {
    ...
    for (i = 0; i < vps->vps_num_hrd_parameters; i++) {
      ...
      if (i) common_inf_present = get_bits1(gb);
  ...
  if (get_bits_left(gb) < 0) {
    av_log(avctx, AV_LOG_ERROR, "Overread VPS by %d bits\n",
           -get_bits_left(gb));
    if (ps->vps_list[vps_id]) goto err;
  }
...
int ff_hevc_parse_sps(HEVCSPS *sps, GetBitContext *gb, unsigned int *sps_id,
  ...
  *sps_id = get_ue_golomb_long(gb);
  ...
  for (i = 0; i < sps->nb_st_rps; i++) {
    if ((ret = ff_hevc_decode_short_term_rps(gb, avctx, &sps->st_rps[i], sps,
                                             0)) < 0)
      return ret;
  }
  ...
  if (vui_present) decode_vui(gb, avctx, apply_defdispwin, sps);
  ...
  if (sps->CtbLog2SizeY < 4) {
    av_log(avctx, AV_LOG_ERROR,
           "log2_ctb_size %d differs from the bounds of any known profile\n",
           sps->CtbLog2SizeY);
    avpriv_request_sample(avctx, "log2_ctb_size %d", sps->CtbLog2SizeY);
    return AVERROR_INVALIDDATA;
  }

  sps->ctb_width =
      (sps->width + (1 << sps->CtbLog2SizeY) - 1) >> sps->CtbLog2SizeY;
  sps->ctb_height =
  ...
  if (sps->log2_max_trafo_size > FFMIN(sps->CtbLog2SizeY, 5)) {
...
int ff_hevc_decode_nal_sps(GetBitContext *gb, AVCodecContext *avctx,
  ...
  if (!sps_buf) return AVERROR(ENOMEM);
...
static void hevc_pps_free(void *opaque, uint8_t *data) {
  ...
  av_freep(&pps->colWidth);
  av_freep(&pps->rowHeight);
  av_freep(&pps->colBd);
  av_freep(&pps->rowBd);
  av_freep(&pps->col_idxX);
  av_freep(&pps->CtbAddrRsToTs);
  av_freep(&pps->CtbAddrTsToRs);
...
static inline int setup_pps(AVCodecContext *avctx, GetBitContext *gb,
  ...
  if (pps->uniform_spacing_flag) {
    if (!pps->colWidth) {
      pps->colWidth =
          av_malloc_array(pps->num_tile_columns, sizeof(*pps->colWidth));
      pps->rowHeight =
          av_malloc_array(pps->num_tile_rows, sizeof(*pps->rowHeight));
    }
  ...
  pps->CtbAddrRsToTs =
      av_malloc_array(pic_area_in_ctbs, sizeof(*pps->CtbAddrRsToTs));
  pps->CtbAddrTsToRs =
      av_malloc_array(pic_area_in_ctbs, sizeof(*pps->CtbAddrTsToRs));
  pps->TileId = av_malloc_array(pic_area_in_ctbs, sizeof(*pps->TileId));
  pps->min_tb_addr_zs_tab =
      av_malloc_array((sps->tb_mask + 2) * (sps->tb_mask + 2),
                      sizeof(*pps->min_tb_addr_zs_tab));
  if (!pps->CtbAddrRsToTs || !pps->CtbAddrTsToRs || !pps->TileId ||
      !pps->min_tb_addr_zs_tab) {
  ...
  for (ctb_addr_rs = 0; ctb_addr_rs < pic_area_in_ctbs; ctb_addr_rs++) {
    ...
    for (i = 0; i < tile_x; i++)
      val += pps->rowHeight[tile_y] * pps->colWidth[i];
    for (i = 0; i < tile_y; i++)
      val += sps->ctb_width * pps->rowHeight[i];

    val += (tb_y - pps->rowBd[tile_y]) * pps->colWidth[tile_x] + tb_x -
           pps->colBd[tile_x];

    pps->CtbAddrRsToTs[ctb_addr_rs] = val;
    pps->CtbAddrTsToRs[val] = ctb_addr_rs;
  }
  ...
  for (y = 0; y < sps->tb_mask + 1; y++) {
    for (x = 0; x < sps->tb_mask + 1; x++) {
      ...
      int val = pps->CtbAddrRsToTs[rs] << (log2_diff * 2);
...
int ff_hevc_decode_nal_pps(GetBitContext *gb, AVCodecContext *avctx,
  ...
  if (pps->tiles_enabled_flag) {
    ...
    if (!pps->uniform_spacing_flag) {
      ...
      for (i = 0; i < pps->num_tile_columns - 1; i++) {
        pps->colWidth[i] = get_ue_golomb_long(gb) + 1;
        sum += pps->colWidth[i];
      }
      ...
      pps->colWidth[pps->num_tile_columns - 1] = sps->ctb_width - sum;

      sum = 0;
      for (i = 0; i < pps->num_tile_rows - 1; i++) {
        pps->rowHeight[i] = get_ue_golomb_long(gb) + 1;
        sum += pps->rowHeight[i];
      }
      ...
      pps->rowHeight[pps->num_tile_rows - 1] = sps->ctb_height - sum;
    }
  ...
  if (log2_parallel_merge_level_minus2 > sps->CtbLog2SizeY) {
    av_log(avctx, AV_LOG_ERROR,
  ...
  if (get_bits1(gb)) { // pps_extension_present_flag
    ...
    if (sps->ptl.general_ptl.profile_idc == FF_PROFILE_HEVC_REXT &&
      ...
      if ((ret = pps_range_extensions(gb, avctx, pps, sps)) < 0) goto err;
    }
  }
