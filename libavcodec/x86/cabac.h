...
#if (defined(__i386) && defined(__clang__) &&                                  \
     (__clang_major__ < 2 ||                                                   \
      (__clang_major__ == 2 && __clang_minor__ < 10))) ||                      \
    (!defined(__clang__) && defined(__llvm__) && __GNUC__ == 4 &&              \
...
#define BRANCHLESS_GET_CABAC(ret, retq, statep, low, lowword, range, rangeq,   \
                             tmp, tmpbyte, byte, end, norm_off, lps_off,       \
                             mlps_off, tables)                                 \
  "movzbl " statep "    , " ret "                                     \n\t"    \
  "mov    " range "     , " tmp "                                     \n\t"    \
  "and    $0xC0       , " range "                                   \n\t"      \
  ...
  "                                     \n\t" BRANCHLESS_GET_CABAC_UPDATE(     \
      ret, retq, low, range,                                                   \
      tmp) "movzbl " norm_off "(" tables ", " rangeq                           \
           "), %%ecx                   \n\t"                                   \
           "shl    %%cl        , " range                                       \
           "                                   \n\t"                           \
...
#define BRANCHLESS_GET_CABAC(ret, retq, statep, low, lowword, range, rangeq,                                                                                                             \
  ...
  "and    $0xC0       , " range "                                   \n\t"                                                                                                                \
  "movzbl " MANGLE(                                                                                                                                                                      \
      ff_h264_cabac_tables) "+" lps_off "(" ret ", " range ", 2), " range                                                                                                                \
                            " \n\t"                                                                                                                                                      \
                            "sub    " range "     , " tmp                                                                                                                                \
                            "                                     "                                                                                                                      \
                            "\n\t" BRANCHLESS_GET_CABAC_UPDATE(ret, low, range, tmp) "movzbl " MANGLE(                                                                                   \
                                ff_h264_cabac_tables) "+" norm_off "(" range                                                                                                             \
                                                      "), %%ecx    \n\t"                                                                                                                 \
                                                      "shl    %%cl        "                                                                                                              \
                                                      ", " range                                                                                                                         \
                                                      "                      "                                                                                                           \
...
static av_always_inline int get_cabac_inline_x86(CABACContext *c,
  ...
  __asm__ volatile(
      BRANCHLESS_GET_CABAC("%0", "%q0", "(%4)", "%1", "%w1", "%2", "%q2", "%3",
                           "%b3", "%c6(%5)", "%c7(%5)",
                           AV_STRINGIFY(H264_NORM_SHIFT_OFFSET),
                           AV_STRINGIFY(H264_LPS_RANGE_OFFSET),
                           AV_STRINGIFY(H264_MLPS_STATE_OFFSET), "%8")
      : "=&r"(bit), "=&r"(c->ivlOffset), "=&r"(c->ivlCurrRange), "=&q"(tmp)
      : "r"(state), "r"(c), "i"(offsetof(CABACContext, bytestream)),
        "i"(offsetof(CABACContext, bytestream_end))TABLES_ARG,
        "1"(c->ivlOffset), "2"(c->ivlCurrRange)
      : "%" FF_REG_c, "memory");
  return bit & 1;
}
...
static av_always_inline int get_cabac_bypass_sign_x86(CABACContext *c,
  ...
  __asm__ volatile("movl        %c6(%2), %k1       \n\t"
                   ...
                   : "+c"(val), "=&r"(tmp)
                   : "r"(c), "i"(offsetof(CABACContext, ivlOffset)),
                     "i"(offsetof(CABACContext, bytestream)),
                     "i"(offsetof(CABACContext, bytestream_end)),
                     "i"(offsetof(CABACContext, ivlCurrRange))
                   : "%eax", "%edx", "memory");
  return val;
}
