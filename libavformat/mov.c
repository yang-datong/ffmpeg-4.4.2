...
static int mov_metadata_hmmt(MOVContext *c, AVIOContext *pb, unsigned len) {
  ...
  n_hmmt = avio_rb32(pb);
  if (n_hmmt > len / 4) return AVERROR_INVALIDDATA;
...
static int mov_read_hdlr(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (title_size > 0) {
    ...
    if (title_str[0]) {
      ...
      av_dict_set(&st->metadata, "handler_name", title_str + off,
...
#define DDTS_SIZE 20
  ...
  if ((ret = ffio_read_size(pb, buf, DDTS_SIZE)) < 0) return ret;
  ...
  st->codecpar->frame_size = (frame_duration_code == 0)   ? 512
...
static int mov_aaxc_crypto(MOVContext *c) {
  if (c->audible_key_size != 16) {
    av_log(c->fc, AV_LOG_FATAL,
           "[aaxc] audible_key value needs to be 16 bytes!\n");
    return AVERROR(EINVAL);
  }

  if (c->audible_iv_size != 16) {
    av_log(c->fc, AV_LOG_FATAL,
           "[aaxc] audible_iv value needs to be 16 bytes!\n");
    return AVERROR(EINVAL);
  }
...
get_current_frag_stream_info(MOVFragmentIndex *frag_index) {
  MOVFragmentIndexItem *item;
  if (frag_index->current < 0 || frag_index->current >= frag_index->nb_items)
    return NULL;
...
static int update_frag_index(MOVContext *c, int64_t offset) {
  ...
  item = av_fast_realloc(c->frag_index.item, &c->frag_index.allocated_size,
...
static int mov_read_colr(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!strncmp(color_parameter_type, "prof", 4)) {
    icc_profile =
        av_stream_new_side_data(st, AV_PKT_DATA_ICC_PROFILE, atom.size - 4);
...
static int mov_read_avid(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!ret) ret = mov_read_extradata(c, pb, atom, AV_CODEC_ID_DNXHD);
  return ret;
}
  ...
  if (c->fc->nb_streams >= 1) {
    ...
    if (par->codec_tag == MKTAG('A', 'V', 'i', 'n') &&
      ...
      case 2:
        ...
        den *= 2;
      case 1:
        c->fc->streams[c->fc->nb_streams - 1]
            ->internal->display_aspect_ratio.num = num;
        c->fc->streams[c->fc->nb_streams - 1]
            ->internal->display_aspect_ratio.den = den;
      default:
        return 0;
      }
    }
...
static void mov_parse_stsd_audio(MOVContext *c, AVIOContext *pb, AVStream *st,
  ...
  if (bits_per_sample &&
      (bits_per_sample >> 3) * (uint64_t)st->codecpar->channels <= INT_MAX) {
  ...
  if (st->codecpar->codec_tag == MKTAG('t', 'm', 'c', 'd')) {
    ...
    if (size > 16) {
      ...
      st->avg_frame_rate.num =
          AV_RB32(st->codecpar->extradata + 8); /* timescale */
      st->avg_frame_rate.den =
          AV_RB32(st->codecpar->extradata + 12); /* frameDuration */
#if FF_API_LAVF_AVCTX
      FF_DISABLE_DEPRECATION_WARNINGS
...
static int mov_read_stsc(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  for (i = sc->stsc_count - 1; i < UINT_MAX; i--) {
    ...
    if ((i + 1 < sc->stsc_count &&
        ...
        sc->stsc_data[i].first < first_min || sc->stsc_data[i].count < 1 ||
        sc->stsc_data[i].id < 1) {
      av_log(c->fc, AV_LOG_WARNING,
             "STSC entry %d is invalid (first=%d count=%d id=%d)\n", i,
             sc->stsc_data[i].first, sc->stsc_data[i].count,
             sc->stsc_data[i].id);
      if (i + 1 >= sc->stsc_count) {
...
static int mov_read_stts(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (duration > 0 && duration <= INT64_MAX - sc->duration_for_fps &&
  ...
  if (duration) st->duration = FFMIN(st->duration, duration);
...
static int mov_read_sdtp(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (sc->sdtp_data) av_log(c->fc, AV_LOG_WARNING, "Duplicated SDTP atom\n");
  av_freep(&sc->sdtp_data);
  sc->sdtp_count = 0;

  sc->sdtp_data = av_malloc(entries);
  if (!sc->sdtp_data) return AVERROR(ENOMEM);
...
static int mov_read_ctts(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  for (i = 0; i < entries && !pb->eof_reached; i++) {
    ...
    if (i + 2 < entries) mov_update_dts_shift(sc, duration, c->fc);
  }
...
static int64_t add_index_entry(AVStream *st, int64_t pos, int64_t timestamp,
  ...
  if (st->nb_index_entries + 1U >= UINT_MAX / sizeof(AVIndexEntry)) return -1;
...
static int64_t add_ctts_entry(MOVStts **ctts_data, unsigned int *ctts_count,
  ...
  if ((unsigned)(*ctts_count) >= UINT_MAX / sizeof(MOVStts) - 1) return -1;
...
static void mov_estimate_video_delay(MOVContext *c, AVStream *st) {
  ...
  if (st->codecpar->video_delay <= 0 && msc->ctts_data &&
    ...
    for (ind = 0; ind < st->nb_index_entries && ctts_ind < msc->ctts_count;
      ...
      if (buf_start == MAX_REORDER_DELAY + 1) buf_start = 0;
...
static void mov_build_index(MOVContext *mov, AVStream *st) {
  ...
  if (sc->elst_count) {
    ...
    if ((empty_duration || start_time) && mov->time_scale > 0) {
      ...
      if (av_sat_sub64(start_time, empty_duration) !=
          start_time - (uint64_t)empty_duration)
        av_log(mov->fc, AV_LOG_WARNING,
...
static int mov_read_tkhd(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (st->id != -1) return AVERROR_INVALIDDATA;
...
static int mov_read_tfhd(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  frag->base_data_offset = flags & MOV_TFHD_BASE_DATA_OFFSET ? avio_rb64(pb)
  ...
  if (frag_stream_info) frag_stream_info->next_trun_dts = AV_NOPTS_VALUE;
...
static int mov_read_trex(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  c->fc->duration =
      AV_NOPTS_VALUE; // the duration from mvhd is not representing the whole file when fragments are used.
...
static int mov_read_trun(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (frag_stream_info) {
    if (frag_stream_info->next_trun_dts != AV_NOPTS_VALUE) {
      ...
      av_log(c->fc, AV_LOG_DEBUG,
             "found mfra time %" PRId64 ", using it for dts\n", pts);
  ...
  if ((uint64_t)st->nb_index_entries + entries >=
  ...
  if (frag_stream_info) frag_stream_info->next_trun_dts = dts + sc->time_offset;
  if (i < entries) {
...
static int mov_read_sidx(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (item_count == 0) return AVERROR_INVALIDDATA;
  ...
  if (!is_complete && (pb->seekable & AVIO_SEEKABLE_NORMAL) &&
    ...
    if (!c->have_read_mfra_size) {
      if ((ret = avio_seek(pb, stream_size - 4, SEEK_SET)) < 0) return ret;
      c->mfra_size = avio_rb32(pb);
      c->have_read_mfra_size = 1;
      if ((ret = avio_seek(pb, original_pos, SEEK_SET)) < 0) return ret;
    }
    if (offset == stream_size - c->mfra_size) is_complete = 1;
  }
...
static int mov_read_smdm(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (sc->mastering) return AVERROR_INVALIDDATA;
...
static int mov_read_st3d(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (sc->stereo3d) return AVERROR_INVALIDDATA;
...
static int mov_read_uuid(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!memcmp(uuid, uuid_isml_manifest, sizeof(uuid))) {
    ...
    if (c->export_xmp) {
      ...
      buffer[len] = '\0';
      av_dict_set(&c->fc->metadata, "xmp", buffer, AV_DICT_DONT_STRDUP_VAL);
    } else {
    ...
    *sc = st->priv_data;
    ...
    if (!(*sc)->cenc.encryption_index) {
      ...
      (*sc)->cenc.encryption_index =
          av_mallocz(sizeof(*frag_stream_info->encryption_index));
      if (!(*sc)->cenc.encryption_index) return AVERROR(ENOMEM);
    }
...
static int mov_read_sample_encryption_info(MOVContext *c, AVIOContext *pb,
                                           MOVStreamContext *sc,
  ...
  *sample = av_encryption_info_clone(sc->cenc.default_encrypted_sample);
  ...
  if (sc->cenc.per_sample_iv_size != 0) {
    if ((ret = ffio_read_size(pb, (*sample)->iv, sc->cenc.per_sample_iv_size)) <
        0) {
  ...
  if (use_subsamples) {
    ...
    (*sample)->subsamples =
        av_mallocz_array(subsample_count, sizeof(*subsamples));
...
static int mov_parse_auxiliary_info(MOVContext *c, MOVStreamContext *sc,
  ...
  if (encryption_index->nb_encrypted_samples) return 0;
  ...
  if (!(pb->seekable & AVIO_SEEKABLE_NORMAL) ||
    ...
    av_log(c->fc, AV_LOG_INFO,
           "Failed to seek for auxiliary info, will only parse senc atoms for "
           "encryption info\n");
    goto finish;
  }
  ...
  for (i = 0; i < sample_count && !pb->eof_reached; i++) {
    ...
    ret = mov_read_sample_encryption_info(
        c, pb, sc, sample, sample_info_size > sc->cenc.per_sample_iv_size);
...
static int mov_read_saiz(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (encryption_index->auxiliary_info_default_size == 0) {
    ret = mov_try_read_block(pb, sample_count,
                             &encryption_index->auxiliary_info_sizes);
    if (ret < 0) {
...
static int mov_read_saio(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  for (i = 0; i < entry_count && !pb->eof_reached; i++) {
    ...
    auxiliary_offsets =
        av_fast_realloc(encryption_index->auxiliary_offsets, &alloc_size,
...
static int mov_read_pssh(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  old_side_data = av_stream_get_side_data(st, AV_PKT_DATA_ENCRYPTION_INIT_INFO,
                                          &old_side_data_size);
  if (old_side_data) {
  ...
  ret = av_stream_add_side_data(st, AV_PKT_DATA_ENCRYPTION_INIT_INFO, side_data,
                                side_data_size);
...
static int mov_read_schm(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (atom.size < 8) return AVERROR_INVALIDDATA;
...
static int mov_read_tenc(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (atom.size < 20) return AVERROR_INVALIDDATA;
...
static int cenc_decrypt(MOVContext *c, MOVStreamContext *sc,
                        AVEncryptionInfo *sample, uint8_t *input, int size) {
  int i, ret;

  if (sample->scheme != MKBETAG('c', 'e', 'n', 'c') ||
      sample->crypt_byte_block != 0 || sample->skip_byte_block != 0) {
    av_log(c->fc, AV_LOG_ERROR,
...
static int cenc_filter(MOVContext *mov, AVStream *st, MOVStreamContext *sc,
  ...
  frag_stream_info =
      get_frag_stream_info(&mov->frag_index, mov->frag_index.current, st->id);
  ...
  if (encryption_index) {
    ...
    if (mov->decryption_key) {
      ...
      size_t size;
      uint8_t *side_data =
          av_encryption_info_add_side_data(encrypted_sample, &size);
      if (!side_data) return AVERROR(ENOMEM);
      ret = av_packet_add_side_data(pkt, AV_PKT_DATA_ENCRYPTION_INFO, side_data,
                                    size);
      if (ret < 0) av_free(side_data);
      return ret;
    }
...
static int mov_read_dops(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if ((ret = ff_alloc_extradata(st->codecpar, size)) < 0) return ret;
...
static int mov_read_dmlp(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (atom.size < 10) return AVERROR_INVALIDDATA;
...
static int mov_read_dvcc_dvvc(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  ret = av_stream_add_side_data(st, AV_PKT_DATA_DOVI_CONF, (uint8_t *)dovi,
                                dovi_size);
  if (ret < 0) {
    av_free(dovi);
    return ret;
  ...
  av_log(c, AV_LOG_TRACE,
         "DOVI in dvcC/dvvC box, version: %d.%d, profile: %d, level: %d, "
...
static const MOVParseTableEntry mov_default_parse_table[] = {
    ...
    {MKTAG('s', 'd', 't', 'p'),
     mov_read_sdtp}, /* independent and disposable samples */
  ...
  while (total_size <= atom.size - 8 && !avio_feof(pb)) {
    ...
    if (atom.size >= 8) {
      ...
      if (((a.type == MKTAG('f', 'r', 'e', 'e') && c->moov_retry) ||
          ...
          a.size >= 8 && c->fc->strict_std_compliance < FF_COMPLIANCE_STRICT) {
        uint32_t type;
        avio_skip(pb, 4);
        type = avio_rl32(pb);
    ...
    if (!parse && c->found_hdlr_mdta &&
        ...
        a.type == MKTAG('k', 'e', 'y', 's') && c->meta_keys_count == 0) {
      parse = mov_read_keys;
    }
...
static int mov_probe(const AVProbeData *p) {
  ...
  for (;;) {
    ...
    if ((offset + 8ULL) > (unsigned int)p->buf_size) break;
    ...
    if (size > INT64_MAX - offset) break;
    offset += size;
  }
  ...
  for (i = 0; i < s->nb_streams; i++) {
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE) {
      ...
      if (st->codecpar->codec_id == AV_CODEC_ID_DVD_SUBTITLE) {
        if ((err = mov_rewrite_dvd_sub_extradata(st)) < 0) goto fail;
      }
    }
...
retry:
  ...
  if (st->discard != AVDISCARD_ALL) {
    ...
    if (mov->dv_demux && sc->dv_audio_container) {
      ...
      ret = avpriv_dv_produce_packet(mov->dv_demux, pkt, pkt->data, pkt->size,
                                     pkt->pos);
      pkt->buf = buf;
      av_packet_unref(pkt);
      if (ret < 0) return ret;
  ...
  if (sc->ctts_data && sc->ctts_index < sc->ctts_count) {
    ...
    if (next_dts >= pkt->dts) pkt->duration = next_dts - pkt->dts;
    pkt->pts = pkt->dts;
  }
...
static int64_t mov_get_skip_samples(AVStream *st, int sample) {
  ...
  if (st->codecpar->codec_type != AVMEDIA_TYPE_AUDIO) return 0;
